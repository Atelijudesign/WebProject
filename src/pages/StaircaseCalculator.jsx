...
// getSafeVal fuera del componente para que no se recree en cada render
const getSafeVal = (val, fallback = 0) => {
  const num = parseFloat(val);
  return isNaN(num) ? fallback : num;
};

// SvgDiagram como componente de módulo (no dentro de StaircaseCalculator)
// para que React reutilice la instancia y no la desmonte/remonte en cada render
function SvgDiagram({ tipoConnection, results, t1Ta1, t1Ta2, t2TcSup, t2TcInf, t2Hc1, t3HHormigon, t3Hc1, t3Ta, t3Tc, huella }) {
  const W = 800, H = 350;
  const xStart = 200, yStart = 80;
  const xEnd = 600, yEnd = 260;

  const safeT1Ta1 = getSafeVal(t1Ta1, 0);
  const safeT1Ta2 = getSafeVal(t1Ta2, 0);
  const safeT2TcSup = getSafeVal(t2TcSup, 0);
  const safeT2TcInf = getSafeVal(t2TcInf, 0);
  const safeT3HHormigon = getSafeVal(t3HHormigon, 0);
  const safeT3Hc1 = getSafeVal(t3Hc1, 0);
  const safeT2Hc1 = getSafeVal(t2Hc1, 0);
  const safeHuella = getSafeVal(huella, 250);
  const safeT3Ta = getSafeVal(t3Ta, 0);

  const s = {
    ta1: safeT1Ta1, ta2: safeT1Ta2,
    tcSup: safeT2TcSup, tcInf: safeT2TcInf,
    hHormigon: safeT3HHormigon, hc1: tipoConnection === 'tipo2' ? safeT2Hc1 : safeT3Hc1,
    huella: safeHuella,
...
export default function StaircaseCalculator() {
  ...
  const results = useMemo(() => {
    ...
    const safeTargetContrahuella = getSafeVal(targetContrahuella, 180) || 180;
    const safeHuella = getSafeVal(huella, 250);
    const safeT1Ta1 = getSafeVal(t1Ta1, 0);
    const safeT1Esp1 = getSafeVal(t1Esp1, 0);
    const safeT1Ta2 = getSafeVal(t1Ta2, 0);
    const safeT1Esp2 = getSafeVal(t1Esp2, 0);
    const safeT2TcSup = getSafeVal(t2TcSup, 0);
    const safeT2TcInf = getSafeVal(t2TcInf, 0);
    const safeT2Hc1 = getSafeVal(t2Hc1, 0);
    const safeT3Ta = getSafeVal(t3Ta, 0);
    const safeT3Esp = getSafeVal(t3Esp, 0);
    const safeT3Tc = getSafeVal(t3Tc, 0);
    const safeT3HHormigon = getSafeVal(t3HHormigon, 0);
    const safeT3Hc1 = getSafeVal(t3Hc1, 0);

    if (tipoConnection === 'tipo1') {
      alturaTotal = ((safeT1Ta1 + (safeT1Esp1 * 0.001)) - (safeT1Ta2 + (safeT1Esp2 * 0.001))) * 1000;
      numeroPeldanos = Math.round(alturaTotal / safeTargetContrahuella);
      contrahuellaCalculada = numeroPeldanos > 0 ? alturaTotal / numeroPeldanos : 0;
      longitudMedia = safeHuella * numeroPeldanos;
    } else if (tipoConnection === 'tipo2') {
      alturaTotal = (safeT2TcSup - safeT2TcInf) * 1000;
      numeroPeldanos = Math.round(alturaTotal / safeTargetContrahuella);
      const alturaRestante = alturaTotal - safeT2Hc1;
      const peldanosRestantes = numeroPeldanos - 1;
      
      contrahuellaCalculada = peldanosRestantes > 0 ? alturaRestante / peldanosRestantes : 0;
      longitudMedia = safeHuella * peldanosRestantes;
    } else if (tipoConnection === 'tipo3') {
      alturaTotal = ((safeT3Ta + (safeT3Esp * 0.001)) - safeT3Tc) * 1000;
      numeroPeldanos = Math.round((alturaTotal - safeT3HHormigon) / safeTargetContrahuella);
      const alturaRestante = alturaTotal - safeT3HHormigon - safeT3Hc1;
      const peldanosRestantes = numeroPeldanos - 1;
    ...
    angulo = safeHuella > 0 ? (Math.atan(contrahuellaCalculada / safeHuella) * (180 / Math.PI)) : 0;
    const chVal = Math.abs(contrahuellaCalculada);
    const comod = (2 * chVal) + safeHuella;
    const segur = chVal + safeHuella;
  ...
  return (
    <div className="bg-gray-50 dark:bg-bim-dark text-gray-900 dark:text-gray-300 font-sans min-h-screen pt-24 pb-12 transition-colors duration-300">
      <div className="max-w-7xl mx-auto px-4 mb-6 flex items-center justify-between flex-wrap gap-3">
        <Link to="/herramientas" className="inline-flex items-center gap-2 text-sm font-bold text-indigo-400 bg-indigo-500/10 hover:bg-indigo-500/20 border border-indigo-500/30 hover:border-indigo-500/50 px-4 py-2 rounded-xl transition-all duration-300 shadow-sm group">
      ...
      <div className="max-w-7xl mx-auto px-4 grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-4 bg-slate-900/70 border border-slate-700/50 rounded-2xl p-6">
          ...
          <div className="grid grid-cols-2 gap-4 mb-6">
             <div>
                ...
                <input type="number" step="1" value={isNaN(targetContrahuella) ? "" : targetContrahuella} onChange={e=>setTargetContrahuella(e.target.value === "" ? "" : parseFloat(e.target.value))} className="w-full bg-slate-800 border border-slate-700 rounded-xl px-3 py-2 text-white" />
             </div>
             <div>
                <label className="block text-xs text-slate-400 mb-1">Huella <span className="text-[10px] text-slate-500">(mm)</span></label>
                <input type="number" step="1" value={isNaN(huella) ? "" : huella} onChange={e=>setHuella(e.target.value === "" ? "" : parseFloat(e.target.value))} className="w-full bg-slate-800 border border-slate-700 rounded-xl px-3 py-2 text-white" />
             </div>
          </div>
            ...
            <div className="space-y-4">
               <div>
                 ...
                 <div className="grid grid-cols-2 gap-4">
                    <input type="number" title="T.A 1 Superior" step="0.001" value={isNaN(t1Ta1) ? "" : t1Ta1} onChange={e => setT1Ta1(e.target.value === "" ? "" : parseFloat(e.target.value))} className="bg-slate-800 border border-slate-700 rounded-xl px-3 py-2 text-white" />
                    <input type="number" title="Espesor Parrilla 1 (mm)" step="1" value={isNaN(t1Esp1) ? "" : t1Esp1} onChange={e => setT1Esp1(e.target.value === "" ? "" : parseFloat(e.target.value))} className="bg-slate-800 border border-slate-700 rounded-xl px-3 py-2 text-white" />
                 </div>
               </div>
               <div>
                 <span className="text-xs text-bim-blue font-bold block mb-2">NIVEL INFERIOR</span>
                 <div className="grid grid-cols-2 gap-4">
                    <input type="number" title="T.A 2 Inferior" step="0.001" value={isNaN(t1Ta2) ? "" : t1Ta2} onChange={e => setT1Ta2(e.target.value === "" ? "" : parseFloat(e.target.value))} className="bg-slate-800 border border-slate-700 rounded-xl px-3 py-2 text-white" />
                    <input type="number" title="Espesor Parrilla 2 (mm)" step="1" value={isNaN(t1Esp2) ? "" : t1Esp2} onChange={e => setT1Esp2(e.target.value === "" ? "" : parseFloat(e.target.value))} className="bg-slate-800 border border-slate-700 rounded-xl px-3 py-2 text-white" />
                 </div>
               </div>
            </div>
          )}
          ...
          {tipoConnection === 'tipo2' && (
            <div className="space-y-4">
               <div>
                 ...
                 <input type="number" step="0.001" value={isNaN(t2TcSup) ? "" : t2TcSup} onChange={e => setT2TcSup(e.target.value === "" ? "" : parseFloat(e.target.value))} className="w-full bg-slate-800 border border-slate-700 rounded-xl px-3 py-2 text-white" />
               </div>
               <div>
                 <label className="text-xs text-slate-400 block mb-1">Cota T.S.P Inferior</label>
                 <input type="number" step="0.001" value={isNaN(t2TcInf) ? "" : t2TcInf} onChange={e => setT2TcInf(e.target.value === "" ? "" : parseFloat(e.target.value))} className="w-full bg-slate-800 border border-slate-700 rounded-xl px-3 py-2 text-white" />
               </div>
               <div>
                 <label className="text-xs text-slate-400 block mb-1">Hc1 - Contrahuella Base (mm)</label>
                 <input type="number" step="1" value={isNaN(t2Hc1) ? "" : t2Hc1} onChange={e => setT2Hc1(e.target.value === "" ? "" : parseFloat(e.target.value))} className="w-full bg-slate-800 border border-slate-700 rounded-xl px-3 py-2 text-white" />
               </div>
            </div>
          )}
          ...
          {tipoConnection === 'tipo3' && (
            <div className="space-y-4">
               <div>
                 ...
                 <div className="grid grid-cols-2 gap-4">
                    <input type="number" title="T.A 1" step="0.001" value={isNaN(t3Ta) ? "" : t3Ta} onChange={e => setT3Ta(e.target.value === "" ? "" : parseFloat(e.target.value))} className="bg-slate-800 border border-slate-700 rounded-xl px-3 py-2 text-white" />
                    <input type="number" title="Espesor Parrilla (mm)" step="1" value={isNaN(t3Esp) ? "" : t3Esp} onChange={e => setT3Esp(e.target.value === "" ? "" : parseFloat(e.target.value))} className="bg-slate-800 border border-slate-700 rounded-xl px-3 py-2 text-white" />
                 </div>
               </div>
               <div>
                 <span className="text-xs text-emerald-500 font-bold block mb-2">INFERIOR (CONCRETO)</span>
                 <div className="grid grid-cols-2 gap-4">
                    <input type="number" title="Cota Losa T.C" step="0.001" value={isNaN(t3Tc) ? "" : t3Tc} onChange={e => setT3Tc(e.target.value === "" ? "" : parseFloat(e.target.value))} className="bg-slate-800 border border-slate-700 rounded-xl px-3 py-2 text-white" />
                    <input type="number" title="Altura Pedestal (mm)" step="1" value={isNaN(t3HHormigon) ? "" : t3HHormigon} onChange={e => setT3HHormigon(e.target.value === "" ? "" : parseFloat(e.target.value))} className="bg-slate-800 border border-slate-700 rounded-xl px-3 py-2 text-white" />
                 </div>
                 <div className="mt-4">
                   <input type="number" title="Hc1 (Contrahuella inicial mm)" step="1" value={isNaN(t3Hc1) ? "" : t3Hc1} onChange={e => setT3Hc1(e.target.value === "" ? "" : parseFloat(e.target.value))} className="w-full bg-slate-800 border border-slate-700 rounded-xl px-3 py-2 text-white" />
                 </div>
               </div>
            </div>
          )}
        ...
        <div className="lg:col-span-8 flex flex-col gap-6">
          ...
          <div className="bg-[#111827] border border-slate-700/50 rounded-2xl p-6 relative overflow-hidden flex-1 min-h-[400px] flex items-center justify-center">
            {/* SVG Visualizer Canvas — componente de módulo, no se desmonta en cada render */}
          <SvgDiagram
            tipoConnection={tipoConnection}
            results={results}
            t1Ta1={t1Ta1} t1Ta2={t1Ta2}
            t2TcSup={t2TcSup} t2TcInf={t2TcInf} t2Hc1={t2Hc1}
            t3HHormigon={t3HHormigon} t3Hc1={t3Hc1} t3Ta={t3Ta} t3Tc={t3Tc}
            huella={huella}
          />
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 mt-12 pt-8 border-t border-slate-800 flex items-center justify-between flex-wrap gap-4">
        <Link to="/herramientas" className="inline-flex items-center gap-2 text-sm font-bold text-indigo-400 hover:text-indigo-300 transition-colors group">
          <i className="fa-solid fa-arrow-left group-hover:-translate-x-1 transition-transform"></i> Volver a Herramientas
        </Link>
